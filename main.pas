...
var
  ...
  i, j, cnt, numSets, start, stop: Integer;
begin
        ...
        begin
          ...
          repeat
            ...
            begin
              ...
              begin
                start:=0; stop:=0;
                start := Pos('(', arrOctgn[i][0])+1;
                stop := Pos('-', arrOctgn[i][0]);
                setNum := Copy(arrOctgn[i][0], start, stop-start);
                break;
              end;
            end;
...
var
  x, y: Integer;
  cycle, title, oSet, cardInSet, tmpStr, searchStr: String;
  found: Boolean;
begin
  result:='';
  Delete(line, 1, 4);
  ...
  begin
    if Copy(aBucket.Items[0],1,1) <> '0' then
      for i:=2 to aBucket.Items.Count - 1 do
        ...
        begin
          aDeck[indexCnt] := Trim(Copy(aBucket.Items[i], 5, 99));
          Inc(indexCnt);
        end;
  end;
          ...
          if comIcons > ' ' then
            repeat
              ...
              if Pos(',',comIcons) > 0 then
                anIcon:=Copy(comIcons, indexCnt+1, Pos(',',comIcons)-(indexCnt+1))
              else
